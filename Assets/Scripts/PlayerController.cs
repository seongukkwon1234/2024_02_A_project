...
public class PlayerController : MonoBehaviour
{
    ...
    private float currnetX = 0.0f;
    private float currnetY = 45.0f;
    ...
    void HandleRotation()
    {
        ...
        if(isFirstPerson)
        {
            transform.rotation = Quaternion.Euler(0.0f, currnetX, 0.0f);
            firstPersonCamera.transform.localRotation = Quaternion.Euler(currnetY, 0.0f, 0.0f);      //1인칭 카메라 수직 회전           
        }
        else
        {
            currnetX += mouseX;
            ...
            currnetY = Mathf.Clamp(currnetY, Y_ANGLE_MIN, Y_ANGLE_MAX);
           
            Vector3 dir = new Vector3(0, 0, -cameraDistance);
            Quaternion rotation = Quaternion.Euler( currnetY, currnetX, 0);
            thirdPersonCamera.transform.position = transform.position + rotation * dir;
            thirdPersonCamera.transform.LookAt(transform.position);      //카메라가 항상 플레이어를 바라보도록 설정 
